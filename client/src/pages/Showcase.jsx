...
function ShowcaseCard({ showcase, onStar, onComment, onDeleteComment, currentUserId }) {
    ...
    const handleSubmitComment = async () => {
        ...
        try {
            await onComment(showcase.id, newComment, user?.fullName || user?.username, user?.imageUrl);
            setNewComment("");
        } finally {
                ...
                <div className="p-5">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-white/5">
                        <div className="flex items-center gap-4">
                            ...
                            >
                                <Star size={16} className={showcase.hasStarred ? "fill-yellow-400" : ""} />
...
function SubmitShowcaseModal({ isOpen, onClose, projects, onSubmit }) {
    ...
    const handleSubmit = async () => {
        ...
        try {
            await onSubmit({
                ...
                ownerGithub: user?.externalAccounts?.find(a => a.provider === "github")?.username || null,
            });
            onClose();
            // Reset state
            setStep(1);
            setSelectedProject(null);
            setFormData({ liveUrl: "", summary: "", template: "general", imageBase64: null });
    ...
    return (
        <Modal isOpen={isOpen} onClose={resetAndClose} title="Showcase Your Project" size="lg">
                ...
                {step === 1 && (
                    ...
                    >
                        <p className="text-slate-400 text-sm mb-4">
                            Select one of your <strong className="text-emerald-400">public GitHub repositories</strong> to showcase:
                        </p>
                        ...
                        <div className="grid gap-3 max-h-80 overflow-y-auto custom-scrollbar pr-2">
                            {projects.length === 0 ? (
                                <div className="text-center py-8 text-slate-500">
                                    ...
                                    <p className="text-sm mt-2">Add a public GitHub repository to your Projects first.</p>
                                </div>
                            ) : (
...
export default function Showcase() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [showcasesResult, mineResult, projectsResult, trendingResult] = results;
            ...
            if (projectsResult.status === 'fulfilled') {
                ...
                const projects = projectsResult.value.data?.data?.projects || projectsResult.value.data?.data || [];
    ...
    const filteredShowcases = showcases.filter((s) => {
        ...
        const matchesTech =
            !techFilter ||
            s.technologies?.some((t) => t.toLowerCase().includes(techFilter.toLowerCase()));
    ...
    const privateProjectCount = myProjects.filter(p => p.githubData?.isPrivate === true).length;
    ...
    const handleStar = async (showcaseId) => {
        try {
            ...
            setShowcases((prev) =>
                prev.map((s) =>
                    s.id === showcaseId
                        ? { ...s, hasStarred: response.data.starred, starCount: response.data.starCount }
                        : s
                )
            );
            setTrending((prev) =>
    ...
    // Handle comment
    const handleComment = async (showcaseId, content, authorName, authorAvatar) => {
        try {
            const response = await showcaseApi.addComment(showcaseId, content, authorName, authorAvatar);
            // Update local state
            const newComment = response.data.data;
            setShowcases((prev) =>
                prev.map((s) =>
                    s.id === showcaseId
                        ? { ...s, comments: [...(s.comments || []), newComment], commentCount: (s.commentCount || 0) + 1 }
                        : s
                )
            );
        } catch (error) {
    ...
    return (
        <div
            ...
            className="min-h-screen bg-slate-950 md:ml-20 overflow-y-auto"
        >
            <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8 pt-20 md:pt-8">
                ...
                <div className="flex gap-2 mb-6 bg-white/5 p-1 rounded-xl w-fit">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${activeTab === "discover"
                                ...
                                {trending.length > 0 && (
                                    <div>
                                        <div className="flex items-center gap-2 mb-4">
                                            ...
                                            <h2 className="text-lg font-bold text-white">Trending This Week</h2>
                                        </div>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-6">
                                    <p className="text-slate-400">
                                        {myShowcases.length} project{myShowcases.length !== 1 ? "s" : ""} showcased
                                    </p>
                                    <Button
                                        onClick={() => setShowSubmitModal(true)}
                                        disabled={availableProjects.length === 0}
                                        title={availableProjects.length === 0 ? "No public projects available to showcase" : ""}
                                    >
                                ...
                                {availableProjects.length === 0 && myProjects.length > 0 && (
                                    <div className="mb-6 p-4 rounded-xl bg-amber-500/10 border border-amber-500/20">
                                        <p className="text-amber-300 text-sm flex items-center gap-2">
                                            ...
                                            <span>
                                                Only <strong>public GitHub repositories</strong> from your Projects can be showcased.
                                                {privateProjectCount > 0 && ` (${privateProjectCount} private repo${privateProjectCount > 1 ? 's' : ''} hidden)`}
                                                {noRepoProjectCount > 0 && ` (${noRepoProjectCount} project${noRepoProjectCount > 1 ? 's' : ''} without GitHub repos)`}
                                            </span>
                                        </p>
                                    </div>
                                )}
                                        ...
                                        {availableProjects.length > 0 ? (
                                            ...
                                            <div className="space-y-2">
                                                <p className="text-slate-500 text-sm">
                                                    You have {myProjects.length} project{myProjects.length > 1 ? 's' : ''}, but none are eligible for showcase.
                                                </p>
                                                <p className="text-slate-500 text-sm">
                                                    Only <strong className="text-purple-400">public GitHub repositories</strong> can be showcased.
                                                </p>
                                            </div>
                                        ) : (
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-slate-500 text-sm">
                                                    Go to <Link to="/projects" className="text-purple-400 hover:text-purple-300 underline font-semibold">Projects</Link> to add a public GitHub repository first.
                                                </p>
                                            </div>
                                        )}
                                    </div>
